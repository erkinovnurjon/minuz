<template>
    <div class="custom-datepicker" :class="{ 'my-datepicker-disabled' : disabled }">
        ...
        >
          <template #input>
              <b-form-input :disabled="disabled" @keyup="$emit('keyup',value)" @input="Change" :placeholder="placeholder" v-mask="'##.##.####'" type="text" maxlength="10" v-model="value" class="my-custom-date" />
          </template>
        </date-picker>
    </div>
  </template>
