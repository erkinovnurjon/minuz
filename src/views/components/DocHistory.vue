    ...
    <b-card-body style="height:600px;overflow-y:auto">
      <app-timeline>
        <app-timeline-item v-for="(item,index) in data" :key="index" :variant="getColor(item)" >
          ...
          <p > <b v-if="!!item.userinfo">{{ item.userinfo }} - </b>  <b v-if="!!item.username"> {{ item.username }} - </b>  <b-badge :variant="getColor(item)"> {{ item.statusname }} </b-badge>  </p>
...
export default {
  ...
  methods:{
      getColor(item){
          ...
          else if(item.statusname === 'Отправлена в школу'){
              return 'warning'
          }
          else if(item.statusname === 'Новая'){
              return 'primary'
          }
          else{
              return 'primary'
