      ...
      <b-col sm="12" md="12" lg="12">
        <b-card>
          ...
          <b-row>
            <b-col sm="12" md="4" lg="4">
              <div class="form-group">
                <label class="col-form-label" for>{{ $t("fromYear") }}</label>
                <div>
                  <b-input-group>
                    <b-form-input
                      :placeholder="$t('fromYear')"
                      v-model="Data.fromYear"
                      v-mask="'####'"
                      value="2021"
                      required
                    />
            ...
            <b-col sm="12" md="4" lg="4">
              <div class="form-group">
                <label class="col-form-label" for>{{ $t("toYear") }}</label>
                <div>
                  <b-input-group>
                    <b-form-input
...
export default {
  ...
  created() {
    ...
    EduYearService.Get(this.$route.params.id)
      .then((res) => {
        this.loading = false;
        this.Data = res.data;
  ...
  methods: {
    check() {
      ...
      if (
        self.Data.fromYear === 0 ||
        self.Data.fromYear === null ||
        self.Data.fromYear === undefined ||
        self.Data.fromYear === ""
      ) {
        this.makeToast(this.$t("fromYearNotCorrect"), 400);
        return false;
      }
      if (
        self.Data.toYear === 0 ||
        self.Data.toYear === null ||
        self.Data.toYear === undefined ||
        self.Data.toYear === ""
      ) {
        this.makeToast(this.$t("toYearNotCorrect"), 400);
        return false;
      }
      // if (
      //   self.Data.isCurrent === 0 ||
      //   self.Data.isCurrent === null ||
      //   self.Data.isCurrent === undefined ||
      //   self.Data.isCurrent === ""
      // ) {
      //   this.makeToast(this.$t("isCurrentNotCorrect"), 400);
      //   return false;
      // }
      if (self.Data.id !== 0) {
        if (
    ...
    SaveData() {
      ...
      EduYearService.Update(this.Data)
        .then((res) => {
          this.makeToast(this.$t("SuccessMessage"), 200);
          this.$router.push({ path: "/info/eduyear" });
