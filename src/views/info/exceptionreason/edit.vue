      ...
      <b-col sm="12" md="12" lg="12">
        <b-card>
          ...
          <b-row>
            <b-col sm="12" md="4">
              <div class="form-group">
                <label class="col-form-label" for>{{ $t("Detail") }}</label>
                <div>
                  <b-input-group>
                    <b-form-input
...
import ExceptionReasonService from "@/services/info/exceptionreason.service";
...
export default {
  ...
  created() {
    ...
    ExceptionReasonService.Get(this.$route.params.id)
      .then((res) => {
        this.loading = false;
        this.Data = res.data;
        if (this.$route.params.id == 0) {
          this.Data.eduYearId = null;
        }
      })
      .catch((error) => {
    ...
    ManualService.ConditionTypeSelectList().then((res) => {
      this.ConditionTypeList = res.data;
    });
    EduYearService.GetAsSelectList()
      .then((res) => {
        this.EduYearList = res.data;
      })
      .catch((error) => {
        this.makeToast(error.response.data, 400);
        this.loading = false;
      });
  ...
  methods: {
    check() {
      ...
      if (
        ...
        this.makeToast(this.$t("DetailNotSelect"), 400);
        return false;
      }
      return true;
    },
    ...
    SaveData() {
      ...
      ExceptionReasonService.Update(this.Data)
        .then((res) => {
          this.makeToast(this.$t("SuccessMessage"), 200);
          this.$router.push({ path: "/info/exceptionreason" });
