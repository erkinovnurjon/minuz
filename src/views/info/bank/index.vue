          ...
          <template #cell(actions)="{ item }">
            <div class="text-center">
              <b-link
                ...
                :to="{ name: 'EditBank', params: { id: item.id } }"
...
export default {
  ...
  created() {
    if (this.$store.state.BankFilter != undefined) {
      this.filter = this.$store.state.BankFilter;
    }
    this.Refresh();
  },
  methods: {
    ...
    Edit(item) {
      this.$store.state.BankFilter = this.filter;
      this.$router.push({ path: "/info/bank/edit/" + item.id });
    },
    Refresh() {
      this.isBusy = true;
      BankService.GetList(this.filter).then((res) => {
